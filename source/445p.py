...
def write_weird_file(fileName, inputSentence, mode="w", sort_first=False, reverse_first=False):
    outputFile = open(fileName, mode)
    if sort_first == False and reverse_first == False:
        for line in inputSentence:
            outputFile.write(str(line) + "\n")
    elif sort_first == False and reverse_first == True:
        inputSentence.reverse()
        for line in inputSentence:
            outputFile.write(str(line) + "\n")
        
    elif sort_first == True and reverse_first == False:
        inputSentence.sort()
        for line in inputSentence:
            outputFile.write(str(line) + "\n")
        
    elif reverse_first == True and sort_first == True:
        inputSentence.sort()
        inputSentence.reverse()
        for line in inputSentence:
            outputFile.write(str(line) + "\n")
...
#write_weird_file("output.txt", ["Wait, where'd the other text go?", "It's gonffafe!"])
write_weird_file("output.txt", [44, 58, 24, 91, 98, 35], mode="a", sort_first=False, reverse_first=True)
