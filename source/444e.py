...
def find_net_force_from_file(fileName):
    ...
    for angles in fileOpen:
        forces.append(angles.split())
    ...
    for force in forces:
        ...
        horizontal = float(magnitude) * cos(angle)
        vertical = float(magnitude) * sin(angle)
    ...
    fileOpen.close()
    return round(net_magnitude, 1), net_angle
