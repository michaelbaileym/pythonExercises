...
def string_finder(targetString, searchString):
    splitTarget = targetString.split()
    searchLen = len(searchString)
    if searchString in targetString:
        #two word string
        if len(splitTarget) > 1:
            if splitTarget[0] == searchString:
                return "Beginning"
            elif splitTarget[0].endswith(searchString):
                return "Middle"
            elif splitTarget[1].endswith(searchString):
                return "End"
        #one word string
        elif len(splitTarget) == 1:
            if splitTarget[0][0:searchLen] == searchString:
                return "Beginning"
            elif splitTarget[0].endswith(searchString):
                return "End"
            else:
