...
def course_info(inputTuple):
    courseDict = {}
    avg_age = 0

    for i in range(len(inputTuple)):
        if "students" in courseDict:
            courseDict["students"] += [inputTuple[i][0]]
        else:
            courseDict["students"] = [inputTuple[i][0]]
        avg_age += inputTuple[i][1]
    avg_age /= len(inputTuple)
    courseDict["avg_age"] = avg_age
    return courseDict
